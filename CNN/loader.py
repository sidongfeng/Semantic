...
def data_generator(batch_sz=32):
    ...
    train_input = np.array([np.array(t).reshape(-1) for t in train_input]).reshape(-1,1,VECTOR_DIM,VECTOR_DIM)
    ...
    valid_input = np.array([np.array(t).reshape(-1) for t in valid_input]).reshape(-1,1,VECTOR_DIM,VECTOR_DIM)
    ...
    test_input = x_test[:,0]
    test_input = np.array([np.array(t).reshape(-1) for t in test_input]).reshape(-1,1,VECTOR_DIM,VECTOR_DIM)
