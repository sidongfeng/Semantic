...
myFPtree, myHeaderTab = createFPtree(initSet, 0.05) # 最小支持度3
freqItems = []
mineFPtree(myFPtree, myHeaderTab, 0.05, set([]), freqItems)
freqItems = [x for x in freqItems if len(x)<=2]

# compute support values of freqItems
suppData = calSuppData(myHeaderTab, freqItems, len(simDat))
suppData[frozenset([])] = 0.05

freqItems = [frozenset(x) for x in freqItems]
print(generateRules(freqItems, suppData))
